...
# ============ DETECT CELLS ============
def detect_cells(image, magnification):
    """Run YOLOv8 detection"""
    if model is None:
        st.error("Model not loaded")
        return []
    
    img_resized = image.resize((1024, 1024))
    results = model.predict(img_resized, conf=0.3, iou=0.5, verbose=False)
...
if st.session_state.image:
    if st.button("🔍 Detect cells", use_container_width=True, type="primary"):
        with st.spinner("Running detection..."):
            st.session_state.detections = detect_cells(st.session_state.image, magnification)
