...
def load_model():
    ...
    try:
        ...
        st.error("❌ Model not found. Configure GDRIVE_FILE_ID in secrets.")
        return None
...
if uploaded and model:
    ...
    if st.button("🔍 Detect", use_container_width=True, type="primary"):
        ...
        # Detect
        results = model.predict(img_canvas, conf=conf, iou=0.5, verbose=False)
        detections = []
        ...
        for result in results:
            for box in result.boxes:
                ...
                # Scale back
                x1 = (x1 - x_off) * (image.width / w)
                y1 = (y1 - y_off) * (image.height / h)
                x2 = (x2 - x_off) * (image.width / w)
                y2 = (y2 - y_off) * (image.height / h)
